...
필수 배경: superpowers:test-driven-development를 반드시 먼저 이해해야 한다. 그 스킬이 RED-GREEN-REFACTOR 사이클의 기초를 정의한다.
...
```markdown
필수 선행: superpowers:test-driven-development를 반드시 이해해야 한다
```
