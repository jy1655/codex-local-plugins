...
| 5. handoff | `codex-plan-review` 또는 구현 착수 추천 |
...
- 왕복이 필요하면 이렇게 짧게 유도한다:
  - "이건 Plan Mode가 맞습니다. `Shift+Tab`으로 Plan Mode로 바꾼 뒤 `/codex-office-hours`를 다시 실행하세요."
...
- 결과는 구현으로 바로 가지 말고 다음 단계가 `codex-plan-review`인지 구현인지 명확히 남긴다.
...
6. 다음 단계로 `codex-plan-review` 또는 구현 착수를 추천한다.
