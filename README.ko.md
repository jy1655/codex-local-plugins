...
- 관리되는 hook entry는 `~/.codex/hooks.json`에 병합됩니다
...
- repo-managed hook behavior는 `hooks/`와 `plugins/jy-env-core/hooks/`에서 작성합니다. 생성된 `~/.codex/hooks.json` entry를 직접 수정하지 않습니다.
